...
Note that we should take to use idiomatic pattern to make sure synthesis use 
the available hardware (BRAM blocks). The original book implementation use an 
...
Here's an example implementation of a logic array. We want to produce the 
...
![Logic array](./ch05/logic_array.svg)

Theses kind of logic array made logic programmable, software tool allow user to 
map logic design into theses arrays. Most logic array are also reconfigurable. 
Logic array are divided in 2 types: PLAs (Programmable logic array) and FPGAs 
(Field programmable gate arrays). PLA is the old technology, it can perform 
only combinational logic, FPGA can perform both combinational and sequential 
logic.

PLA use a network of AND array connected to a network of OR array, this is used 
to produce the expected output. It has been largely displaced by FPGAs.

FPGAs are built as an array of *logic elements (LE)*, also refered as 
*configurable logic block (CLB)*. They can be configured to perform 
combinational or sequential logic. FPGAs also have *Input/Output element (IOE)* 
for interfacing with the outside world. The key elements of the LE are: 4 input 
LUT (Lookup-Table), 1 bit register, configurable mux, 1 output.

The LE's LUT contain enough bit to produce any combination of a 4:1 truth 
table. 

More details are device specific, and can be found on the manufacturer 
specification.
