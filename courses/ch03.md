...
WARNING: S1 and S2 represent bit 1 (LSB) of the **ENCODING**, not state 1.
| S1 | S2 | Ta | Tb | S'1 | S'2 |
|----|----|----|----|-----|-----|
|  0 |  0 |  0 |  X |  0  |  1  |
|  0 |  0 |  1 |  X |  0  |  0  |
...
When a flip-flop copies the input D to the output Q on the rising edge of the 
clock we call this process *sampling*. If D is *stable* at 0 or 1 when the 
clock rise the behavior is clearly defined, but if D is changing at the same 
time this cause issue.

A sequential element have an *aperture time* around the clock edge, during 
which the input must be stable. It's defined by a *setup time* and a *hold 
time*. 

Just like the static discipline limited us to use logic level outside the 
forbidden zone, the **dynamic discipline** limits us to using signals that can
change only outside the apperture time.

A signal can change wildly between apperture time, we're only interested in 
it's value at the end of the clock cycle. We write `A[n]` the signal `A` at the 
end of the clock cycle `n`.

That mean the clock cycle has to be long enough for all signal to settle. This 
set a limit on the speed of the system.

In real system, the clock doesn't reach all flip-flop at the same time, this 
variation is called *clock skew* and increase the necessary clock period.

Sometimes, especially when interfacing with the real world, it's impossible 
to satisfy the dynamic discipline. A user button can be pressed in the 
aperture time, we're ending up with a flip-flop containing a value partway 
between 0 and 1 (*metasability*), which cause issues. The solution is to use 
*synchronizer* which have very small, but nonzero, probability of producing an
illegal value.

## System timing

Expressed in Hz, the clock period or cycle time is the between rising edge of a 
repetive clock signal. Considere a simple circuit synchronous sequential 
circuit.

![Register timing schematic](./ch03/register_timing_schematic.svg)

For this circuit, the following timing diagram describe the sequence. White 
arrow represent *contamination delay* while blue arrow represent *propagation 
delay*. Here are the flip-flop clock-to-Q delays and the combinatial logic 
delays.

![Register timing](./ch03/register_timing_annotated.png)

Time constraint can be represented with the following equation :
`Tc > T(clock-to-Q) + T(combinational logic) + Tsetup`. Recall that `Tsetup` is
a part of the aperture time of sequential circuit, this is the time the signal 
must be stabilized before the clock edge. The other timing are the **propagation 
delay**. This is called the *setup time constraint* or the *max-delay 
constraint*. Note that propagation delay of combinational is just a part of the 
available time in clock period, `T(clock-to-Q)` and `Tdelay` is called 
*sequencing overhead*.

On the other side, we must take care of respecting `Thold` constraint, the 
general rules is that `T(clock-to-Q) + T(combinational logic) > Thold`: we 
must not be too quick to do the combinational logic. This can be transformed to 
`T(combination logic) >= Thold - T(clock-to-Q)`. T here is **contamination 
delays**. However flip-flop are often designed so that there can be no 
combinational logic between them (`Thold < T(clock-to-Q)`), but this must be 
checked in the manufacturer specification.

## Clock skew

The variation of time of clock edge between registers is called clock skew. 
Wire length only suffice to produce clock skew, but also clock gating. This 
skew factor must be added to the previous equations as :
- `Tc > T(clock-to-Q) + T(combinational logic) + Tsetup + Tskew`
- `T(combination logic) >= Thold + Tskew - T(clock-to-Q)`

`Tskew` effectively increase both setup time and hold time, it add to the 
sequencing overhead, reducing time available for combinational logic. It also 
increase the minimum contamination delay required to combinational logic to 
respect Thold constraint. Even if flip-flops are designed with `Thold < 
T(clock-to-Q)` to support back-to-back operations, if clock skew is too large 
this will cause *hold time violation*.

Designer must carefully reduce as much as possible clock skew.

## Metastable states

Every bistable can have a metastable state between 0 and 1. It can happen when 
the input is not stable during the aperture time. Eventually, the flip-flop 
will resolv the output to a stable state, of either 0 or 1, however the 
*resolution time* can be infinite.

A metastable flip-flop is analogous to a ball on the summit of a hill between 
two valleys. Valleys are stable state. The ball would remain at the top if it's 
perfectly balanced, but as nothing is perfect in this world, it will surely 
fall into one valley, but it can take up to an infinite amount of time.

To counter metastable and to support asynchronous inputs we use 
*synchronizers*. It's principle is that we reduce the amount of probability to 
*still* have a metastable input by waiting (by resampling the saved state with 
another register). Depending on the system purpose, we may set the threshold of 
"acceptable probability of failure" at different levels.

## Parallelism

The speed of a system is defined by the latency and throughput of information 
passing through. Let a *token* be a group of inputs that are processed to 
produce a group of outputs. The *latency* of the system is the time required 
for one token to pass through the system from start to end. The *throughput* 
is the number of token that can be processed per unit time.

Throughput can be improved by processing multiples tokens at the same time, 
this is called *parallelism*, which come in 2 forms: *spatial* and *temporal*.

With *spatial paralelism* multiple copy of the hardware exist so that multiple 
tasks can be done at the same time. With *temporal parallelism*, tasks is 
broken into stages: althought each tasks must pass through all stage, a 
different tasks will be in each stage at any given task, so tasks can overlap.

Temporal parallelism is attractive because it can enable better performance 
without multiplying the hardware. The bane of parallelism is *dependencies*, if 
a current task is dependent of the result of a prior task, the whole task 
cannot start until the prior task has completly completed.
