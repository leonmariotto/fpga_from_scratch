...
To counter metastable and to support asynchrnous inputs we use *synchronizers*.
It's principle is that we reduce the amount of probability to *still* have a 
metastable input by waiting (by resampling the saved state with another 
register). Depending on the system purpose, we may set the threshold of 
