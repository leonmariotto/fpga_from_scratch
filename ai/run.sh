...
# Constants
readonly project_name="fpga"
readonly codex_runtime_auth_loc="/home/ubuntu/.codex/auth.json"
...
function main() {
    ...
    agent_file="${script_dir}/agents/default.md"
    skills_dir="${script_dir}/skills"
    auth_loc="${script_dir}/cache/auth.json"
    if [[ ! -f "${auth_loc}" ]]; then
        # If auth.json don't exist login first. Next call should use the
        # mounted auth.json.
        touch "${auth_loc}"
        docker run -it --rm \
            --user "$(id -u):$(id -g)" \
            -v ${project_dir}:/workspace/${project_name} \
            -v "$HOME/.cache/uv:/home/ubuntu/.cache/uv" \
            -v ${skills_dir}:/home/ubuntu/.agents/skills \
            -v ${auth_loc}:${codex_runtime_auth_loc} \
